// Based on dart-lang/sdk/lib/_internal/vm/lib/convert_patch.dart
//
// Copyright (c) 2014, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.

// ignore_for_file: slash_for_doc_comments, unnecessary_this, constant_identifier_names, unused_field, annotate_overrides, dead_code, avoid_return_types_on_setters
...
abstract class _ChunkedJsonParser<T> {
    ...
    while (position < length) {
      ...
      switch (char) {
        ...
        case COMMA:
          ...
          break;
        case RBRACKET:
          if (state == STATE_ARRAY_EMPTY || state == STATE_ARRAY_COMMA) {
            listener.endArray();
          } else if (state == STATE_ARRAY_VALUE) {
            listener.arrayElement();
            listener.endArray();
          ...
          break;
        case RBRACE:
          if (state == STATE_OBJECT_EMPTY || state == STATE_OBJECT_COMMA) {
            listener.endObject();
          ...
          break;
        case SLASH:
          position = parseComment(position);
          break;
        default:
          if ((state & ALLOW_VALUE_MASK) != 0) fail(position);
          state |= VALUE_READ_BITS;
  ...
  int parseComment(int position) {
    assert(getChar(position) == SLASH);
    if (chunkEnd < position + 2) {
      return position;
    }
    int char = getChar(position + 1);
    if (char == SLASH) {
      return parseLineComment(position);
    } else if (char == STAR) {
      return parseBlockComment(position);
    } else {
      fail(position);
    }
  }

  int parseLineComment(int position) {
    assert(getChar(position) == SLASH);
    assert(getChar(position + 1) == SLASH);
    int length = chunkEnd;
    position += 2;
    while (position < length) {
      int char = getChar(position);
  ...
  int parseBlockComment(int position) {
    assert(getChar(position) == SLASH);
    assert(getChar(position + 1) == STAR);
    int length = chunkEnd;
    position += 2;
    while (position < length) {
      int char = getChar(position);
